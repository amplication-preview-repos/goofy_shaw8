/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { VisitCounterWhereInput } from "./VisitCounterWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { VisitCounterOrderByInput } from "./VisitCounterOrderByInput";

@ArgsType()
class VisitCounterFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => VisitCounterWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => VisitCounterWhereInput, { nullable: true })
  @Type(() => VisitCounterWhereInput)
  where?: VisitCounterWhereInput;

  @ApiProperty({
    required: false,
    type: [VisitCounterOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [VisitCounterOrderByInput], { nullable: true })
  @Type(() => VisitCounterOrderByInput)
  orderBy?: Array<VisitCounterOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { VisitCounterFindManyArgs as VisitCounterFindManyArgs };
